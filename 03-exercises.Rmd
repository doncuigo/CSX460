...
stepAIC(fit.full, scope = form, direction = "backward")
```

- Are they the same model? If not why?  Which is better? Justify your answer.

These are not the same model. The forward stepwise model featured 41 variables, while the backward stepwise model featured 44 variables. The forward model appears to be better, as it ended on a slightly lower AIC (2662.7 vs the backwards model's 3663.2) while also being simpler, as it features fewer variables.
...
logistic <- logistic <- function(x){
  1/(1+exp(x*-1)) 
}

curve(logistic(x),xlim = c(-10,10))

logistic_inv <- function(x){
  -1*log((1/x)-1)
  }

curve(logistic_inv(x),xlim = c(0,1))
...
#glm = generalized linear models; build the best model I can, person who builds the best model will get a prize

Train <- createDataPartition(GermanCredit$Class, p=0.6, list=FALSE)
training <- GermanCredit[ Train, ]
testing <- GermanCredit[ -Train, ]

mod_fit <- train(Class ~ Age + NumberExistingCredits + CheckingAccountStatus.lt.0 + SavingsAccountBonds.lt.100 + EmploymentDuration.lt.1,  data=training, method="glm", family="binomial")
...
#can also use stepAIC(fit.glm,scope, direction...) - see ?stepAIC; don't forget library(MASS)
